const section2Data = [
  {
    id: 1,
    style:"bg-primary text-black",
    title: "Vedara Consultancy",
    para1: "Vedara Consultancy partners with founders, leadership teams, and growing brands to bring clarity to complex decisions. We work at the intersection of business, brand, and growth, helping organizations define direction, align teams, and build strategies that scale.",
    para2: "Our role is not advisory at a distance. We engage deeply, challenge assumptions, and help leaders move forward with confidence and structure.",
    subtitle: "Ideal For",
    para3: "Brands at a crossroads, entering new markets, restructuring, or preparing for long-term scale.",
    btnText: "Request a Private Introduction",
    btnLink: "/contact",
    imgSrc: "/home/frame1.png",
  },

  {
    id: 2,
    style:"bg-black text-white",
    title: "Vedara Atelier",
    para1: "Vedara Atelier transforms strategy into execution. We design and build brand identities, marketing systems, and digital products that are not only visually refined but strategically grounded.",
    para2: "From early-stage brands to established organizations, our atelier creates cohesive experiences that communicate clearly, perform effectively, and grow with the business.",
    subtitle2: "Capabilities",
    points: ["Brand identity & visual systems", "Marketing strategy & communication design", "Websites & digital products", "Design-driven development & execution"],
    subtitle: "Ideal For",
    para3: "Brands that need execution aligned with strategy not disconnected creative output.",
    btnText: "Enter Atelier",
    btnLink: "/atelier",
    imgSrc: "/home/frame2.png",
  },

  {
    id: 3,
    style: "bg-[#2A2927] text-white",
    title: "Vedara Dynasty",
    para1: "It is Vedara’s dynasty and ecosystem-building arm. It focuses on supporting brands through strategic investment, partnerships, and long-term growth frameworks, extending beyond traditional consulting or studio services.",
    para2: "Dynasty is built for brands with ambition: those looking to scale sustainably, collaborate strategically, and become part of a larger brand ecosystem.",
    subtitle2: "Capabilities",
    points: ["Dynasty-backed brand growth", "Strategic partnerships & collaborations", "Long-term, scalable brand ecosystems"],
    subtitle: "Note",
    para3: "Dynasty is currently evolving and will expand into a dedicated platform.",
    btnText: "Enter Dynasty",
    btnLink: "/dynasty",
    imgSrc: "/home/frame3.png",
  }
];
export default section2Data;